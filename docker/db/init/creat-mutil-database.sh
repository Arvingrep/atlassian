...
_create_atlassian_database() {
  docker_process_sql --database=mysql <<-EOSQL
    CREATE DATABASE \`$1\` CHARACTER SET utf8mb4 COLLATE utf8mb4_bin;
	GRANT ALL PRIVILEGES ON \`$1\`.* TO '$DATABASE_USERNAME'@'%' IDENTIFIED BY '$DATABASE_PASSWORD' ;
	flush privileges;
EOSQL
}

_create_bitbutket_database() {
  docker_process_sql --database=mysql <<-EOSQL
...
mysql_note "Creating atlassian databases"
_create_bitbutket_database $DATABASE_NAME_SITE_C
_create_bitbutket_database $DATABASE_NAME_SITE_A
for DATABASE_NAME in  $DATABASE_NAME_SITE_B  ; do
  mysql_note "Creating ${DATABASE_NAME}"
  _create_atlassian_database $DATABASE_NAME
